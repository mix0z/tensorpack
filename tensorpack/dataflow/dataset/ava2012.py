...
class AVA2012Meta(object):
    ...
    def get_image_list(self, name, delta_moderate=0, raw_score_flag=False):
        ...
        assert len(ret), \
            "*** Failed to load a list of (img_name, aesthetic_score) out of {0} or delta_moderate {1} is too large ***".format(
                fname, delta_moderate)
    ...
    def __str__(self):
        return "==> This is an instance of AVA2012 data iterator ...\n" + \
               ...
               "    number of pos    : {0} ({1:.3f})\n".format(self.num_pos,
                                                               (self.num_pos) / (self.num_pos + self.num_neg)) + \
               "    number of neg    : {0} ({1:.3f})\n".format(self.num_neg,
                                                               (self.num_neg) / (self.num_pos + self.num_neg)) + \
